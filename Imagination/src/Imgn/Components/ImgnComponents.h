...
namespace Imgn
{
    ...
    struct CameraComponent : public Component
    {
        ...
        CameraComponent(float pFOV = Math::Radians(45.f), float pNearPlane = .1f, float pFarPlane = 1000.f) : Component("Camera")
        {
            cameraType = CameraType::Perspective;
            camera.CreatePerspectiveCamera(pFOV, pNearPlane, pFarPlane);
        }

        CameraComponent(float pRight, float pLeft, float pTop, float pBottom, float pNear = -1.f, float pFar = 1.f) : Component("Camera")
        {
            cameraType = CameraType::Orthographic;
            camera.CreateOrthographicCamera(pRight, pLeft, pTop, pBottom, pNear, pFar);
        }
    };
